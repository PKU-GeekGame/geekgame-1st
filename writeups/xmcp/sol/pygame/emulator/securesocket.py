...
class SecureSocket:
    ...
    def send(self, b):
        ...
        self.sock.send(struct.pack('!i', size))
    ...
    def handshake(self, self_privkey, peer_pubkey):
        ...
        # exchange rsa pubkey
        self.sock.send(self_privkey.public_key().n.to_bytes(1024//8, byteorder='big'))
        recv_pubkey = int.from_bytes(self.sock_r.read(1024//8), 'big')
        if peer_pubkey is None:
            peer_pubkey = RSA.construct((recv_pubkey, 65537))
        else:
            assert peer_pubkey.n==recv_pubkey

        # send chacha key under rsa
        encrypted_key = PKCS1_OAEP.new(peer_pubkey).encrypt(self.send_key)
        self.sock.send(struct.pack('!H', len(encrypted_key)))
        self.sock.send(encrypted_key)

        # recv peer chacha key
...
def test():
    ...
    PSK = b'\x12\x34\x56\x78'*8

    s = socket.socket()
    s.bind(('127.0.0.1', 12321))
    s.listen()
    ...
    def try_connect():
        nonlocal ss2
        ss2 = connect(('127.0.0.1', 12321), PSK, None)
    t = threading.Thread(target=try_connect)
    t.start()
    s1, _addr = s.accept()
    ss1 = SecureSocket(s1, PSK, None, None)
    t.join()
    ...
    for _ in range(10):
        if random.random()>.5:
            ss1.send(b'hello world')
            assert ss2.recv(None)==b'hello world'

        if random.random()>.5:
            ss2.send(b'wow')
            assert ss1.recv(None)==b'wow'
